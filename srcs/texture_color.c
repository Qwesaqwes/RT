...
/*   Updated: 2017/03/07 00:11:29 by jichen-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// }
//
// double noise_3d(float x, float y, float z)
// {
...
float		perlin_coef(float x, float y, float z)
{
	...
	return (lerp(fade_curves.z, lerp(fade_curves.y, lerp(fade_curves.x, grad(p[AA], x, y, z),
																		grad(p[BA], x - 1, y, z)),
													lerp(fade_curves.x, grad(p[AB], x, y - 1, z),
																		grad(p[BB], x - 1, y - 1, z))),
								lerp(fade_curves.y, lerp(fade_curves.x, grad(p[AA + 1], x, y, z - 1),
																		grad(p[BA + 1], x - 1, y, z - 1)),
													lerp(fade_curves.x, grad(p[AB + 1], x, y - 1, z - 1),
																		grad(p[BB + 1], x - 1, y - 1, z - 1)))));
}

t_color		perlin(t_no no)
{
	float	n;
	float	v1;
	float	v2;
...
t_color		wood(t_no no)
{
	// float	n;
	//
	// n = perlin_coef(no.ip.x, no.ip.y, no.ip.z) * 10;
	// n = n - floor(n);
	// return (color_add(color_fact(no.obj.tex.tex_col1, n), color_fact(no.obj.tex.tex_col2, 1 - n)));
...
t_color		marble(t_no no)
{
	...
	// while (++i < 10)
	// {
	// 	n += (1.0f / (float)i) * fabsf(perlin_coef(no.ip.x * 0.05 * i, no.ip.y * 0.15 * i, no.ip.z * 0.05 * 1));
	// }
	// n = 0.5f * sinf((no.ip.x + no.ip.y) * 0.05f + n) + 0.5f;
	// return (color_add(color_fact(no.obj.tex.tex_col1, n), color_fact(no.obj.tex.tex_col2, 1 - n)));
	float	n;
	float	v1;
	float	v2;
